...
# Display-only overlay: drop the extreme top 1% so the call-side figure isn't
# dominated by a handful of spike outliers. Underlying stats still use best_idx.
...
function path_panel(t_axis, paths_mat, title_str, ylabel_str;
                    tail_idx, tail_color, tail_label,
                    legend_pos = :topleft,
                    overlay_zero = false)
    ...
    for (k, j) in enumerate(tail_idx)
        plot!(p, t_axis, paths_mat[:, j], color = tail_color, lw = 1.0,
              label = (k == 1 ? tail_label : ""))
    end
...
pE1 = path_panel(t_g, Δ_put_plot,
                 "Long-put Δ at K_put",
                 "Δ (long put)";
                 tail_idx = worst_idx, tail_color = COL_WORST,
                 tail_label = "Worst 5% by terminal price",
                 legend_pos = :bottomleft)
pE2 = path_panel(t_g, Δ_call_plot,
                 "Long-call Δ at K_call",
                 "Δ (long call)";
                 tail_idx = best_display_idx, tail_color = COL_BEST,
                 tail_label = "Top 1–5% by terminal price",
                 legend_pos = :topleft)
