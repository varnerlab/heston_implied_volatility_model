...
const RUNOUT=SMOKE ? joinpath(OUT,"smoke") : OUT
mkpath(RUNOUT)
...
contracts=CSV.read(joinpath(OUT,"origin_contracts.csv"),DataFrame)
...
function growth(marginal,n,seed)
    sim=JumpHMM.simulate(marginal,5;n_paths=n,seed)
...
metadata=Dict("paths_per_origin"=>N,"model_sha256"=>(open(sha256,joinpath(OUT,"surface_model.jld2")) |> bytes2hex),
    ...
    "source_sha256"=>Dict(f=>(open(sha256,joinpath(ROOT,f)) |> bytes2hex) for f in
        ["code/scripts/run_chronological_forecasts.jl","code/src/ForecastValidation.jl","code/src/DynamicAblation.jl"]),
