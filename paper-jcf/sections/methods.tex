...
Rather than specifying a separate initial variance $v_0$, we initialized the variance process directly at the per-contract mean-reversion target so that the entry IV surface inherited the smile, skew, and term structure encoded in $\psi$ without a separate IV-surface input at simulation time:
