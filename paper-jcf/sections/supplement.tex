...
\begin{table}[ht]
\centering
\caption{Multi-cell LoRA sweep. Per-cell base, V0 (level-only), and V1 (rank-2 LoRA + level) RMSE on the day's ladder slice, with V1 adapter-norm summaries on the three Dense layers. V1 reduced RMSE on every cell (no overfit) and dominated V0 across the board; improvements scaled with base RMSE. The five GS control cells (bottom block) saw V1 perturbations near zero, the right ``no-op'' behaviour.}\label{tab:lora_sweep}
\resizebox{\textwidth}{!}{%
...
\midrule
\multicolumn{10}{l}{\emph{Control cells (5 lowest base RMSE; near-no-op V1 behaviour)}} \\
