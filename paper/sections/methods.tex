...
\end{equation}
in which $\gamma = 0$ (the aggregate mood multiplier is reserved for forward simulation with simulated state paths) and the per-state level $\theta_{i,\,s_t}$ collapses to a single per-ticker level $\theta_i$ (since the calibration corpus consisted of observation-day IV surfaces rather than state-conditioned histories). Both restrictions were transparent to $\psi$, so the shape function carried over without modification when the full state-and-mood mechanism was reactivated for simulation.
